...
namespace VixenModules.App.VixenScheduler.Controls
{
    ...
    public class TimeFrame : System.Windows.Forms.UserControl
    {
        ...
        internal class _Day : TimeFrameObject
        {
            ...
            public _Day(RectangleF rec, int row)
                : base(rec)
            {
                Row = row;
            }
        ...
        internal class TimeSlot : TimeFrameObject
        {
            ...
            public TimeSlot(int col)
                : this(col, new RectangleF(0, 0, 0, 0))
            {
            }
            public TimeSlot(int col, RectangleF rec)
                : base(rec)
            {
                Column = col;
            }
        ...
        internal class Cell : TimeFrameObject
        {
            ...
            public Cell(RectangleF rec, int x, int y)
                : base(rec)
            {
                X = x;
                Y = y;
            ...
            set
            {
                ...
                if (value != null)
                {
                    for (int i = 0; i < value.Length; i++)
                    {
                        ...
                        {
                            var start = (int)(value[i][a][0].Ticks / t_inc);
                            var end = (int)(value[i][a][1].Ticks / t_inc);

                            for (int j = start; j < end; j++)
                            {

                                CellMatrix[i, j].Selected = true;
        ...
        private void DrawGrid(Graphics g)
        {
            ...
            for (float i = 0; i <= NumTimeIntervals; ++i)
            {
                if ((i % TimeGridVerticalInterval) == 0)
                {
                    ...
                    {
                        g.DrawLine(Pens.Black, Grid.X + (i * x_grid_inc), 0, Grid.X + (i * x_grid_inc), Grid.Y + Grid.Height);
                    }
                    else
                    {
                        g.DrawLine(Pens.Gray, Grid.X + (i * x_grid_inc), 0, Grid.X + (i * x_grid_inc), Grid.Y + Grid.Height);
                    }
                }
            }
            ...
            for (float i = 0; i <= NumDays; ++i)
            {
                g.DrawLine(Pens.Black, 0, Grid.Y + (i * y_grid_inc), Grid.X + Grid.Width, Grid.Y + (i * y_grid_inc));
            }
